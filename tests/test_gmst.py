...
class TestGMSTFunction(unittest.TestCase):
    ...
    def test_gmst(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            gmst(seqfile=self.fasta, output=prefix, faa=True, fnn=True, strand="direct") #test how we are actually going to use the module

            with open(f"{prefix}", 'r') as res, open(f"{prefix}.faa", 'r') as resfaa, open(f"{prefix}.fnn", "r") as resfnn:
                test_results = "".join(res.readlines()[7:]) # the first few lines include a timestamp and the tmpdir, so they will always differ.  skip those lines.
                test_faa = "".join(resfaa.readlines())
                test_fnn = "".join(resfnn.readlines())

            with open(self.results, 'r') as exp, open(self.faa, 'r') as expfaa, open(self.fnn, "r") as expfnn:
